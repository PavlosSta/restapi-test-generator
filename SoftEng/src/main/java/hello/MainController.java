...
public class MainController {
	...
	String allPrices(@RequestParam Map<String, String> params,
		...
		boolean validStart = true, validCount = true, validSort = true, validDist = false, validDate = false,
				validFormat = true;
		Set<String> alreadySeenSort = new HashSet<String>();
		...
		if (validStart && validCount && validSort && validDist && validDate && validFormat) {
			...
			for (Price p : prices) {
				// Find if there is an intersection betwwen the input and the tags found
				if (tags != null)
					inputTags = tags.stream().collect(toSet());
				else
					inputTags = null;

				Product productTemp = productRepository.findById(p.getProductId()).get();
				List<Product_Tag> productTagTemp = productTemp.getTags();
				Set<String> pTags = productTagTemp.stream().map(Product_Tag::getName).collect(toSet());

				Shop shopTemp = shopRepository.findById(p.getShopId()).get();
				List<Shop_Tag> shopTagTemp = shopTemp.getTags();
				Set<String> sTags = shopTagTemp.stream().map(Shop_Tag::getName).collect(toSet());
	...
	String newPrice(@RequestHeader("X-OBSERVATORY-AUTH") String token, @RequestParam Map<String, String> params)
		...
		List<Product_Tag> productTagTemp = productTemp.getTags();
		List<String> pTags = productTagTemp.stream().map(Product_Tag::getName).collect(toList());

		Shop shopTemp = shopRepository.findById(Integer.valueOf(params.get("shopId"))).get();
		List<Shop_Tag> shopTagTemp = shopTemp.getTags();
		List<String> sTags = shopTagTemp.stream().map(Shop_Tag::getName).collect(toList());
